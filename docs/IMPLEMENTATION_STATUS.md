...
Atualizado em 2026-09-12. Greenfield. Este ledger não pode ter linhas removidas para ocultar pendências.
...
|---|---|---:|---|---|---|
| FOUNDATION-001 | Scaffold Next.js App Router, TypeScript strict e Tailwind | M0 | TESTED | build/typecheck | Next 16.3; build de 40 páginas e typecheck verdes |
...
| NOTIFY-002 | EmailProvider isolado, fake dev e Resend produção | M5 | BLOCKED | provider tests/hosted | Adapter testado; Worker permanece sem ativação Resend porque envio real exige domínio, DNS, remetente verificado, `RESEND_API_KEY` e `EMAIL_FROM` |
...
| SEC-004 | Headers, CSP, cookies, CSRF, noindex e validação server-side | M7 | TESTED | hosted security smoke | Cloudflare confirmou CSP, HSTS, frame-ancestors none, nosniff, Permissions-Policy e noindex; rota admin anônima redireciona e cron sem bearer retorna 401 |
...
| OPS-004 | Ambientes development/preview/production separados | M7 | BLOCKED | deployment | Cloudflare Production publicado e autenticado em `lastro.feraimentor.workers.dev`, com callbacks, secrets e cron validados. Preview reativado, domínio/EMAIL_FROM, envio real, hosted E2E integral e restore hospedado ainda pendentes |
...
- A aplicação foi migrada da Vercel para o Cloudflare Workers em 2026-09-12 pelo adapter OpenNext, sem domínio pago. `https://lastro.feraimentor.workers.dev`, `/login` e o redirecionamento anônimo de `/admin/usuarios` responderam corretamente; headers de segurança e os dois schedules foram confirmados. O bundle usa a chave publishable ativa do Supabase, e `MASTER_BOOTSTRAP_EMAIL`/`CRON_SECRET` estão em secrets do Worker.
- O Site URL e o callback Google do Supabase Production apontam para o host Cloudflare. Uma chave `sb_secret_...` exclusiva do backend foi criada e gravada somente como `SUPABASE_SECRET_KEY` do Worker; o valor não entrou no Git. Login Master, `/admin/usuarios` e o job idempotente de prazos foram validados em produção.
- Supabase Preview `iyrmkcuuxhxovsbkcatq` está `INACTIVE`. A restauração foi recusada porque o plano gratuito já mantém os dois projetos permitidos ativos (`LASTRO` e `Agenda Docente`); nenhum projeto alheio foi pausado. Um Preview da aplicação deve ser publicado quando houver vaga ou upgrade, mantendo chaves e callbacks separados de Production.
- A rota Cloudflare elimina a dependência operacional e o 2FA perdido da Vercel e mantém hospedagem HTTPS sem domínio pago. O domínio compartilhado `workers.dev` não pode ser verificado como remetente no Resend, portanto e-mail transacional real continua dependendo de domínio próprio.
- O envio produtivo para terceiros continua bloqueado até existir domínio próprio, DNS/remetente verificado e configuração de `RESEND_API_KEY`/`EMAIL_FROM` no Worker. Também permanecem externos: teste de envio real, Storage hospedado, restore completo no Preview reativado, E2E administrativo com contas Google adicionais e mobile/axe nas jornadas privadas. O site continua com `noindex`, sem alegação de lançamento produtivo.
