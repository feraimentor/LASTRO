...
| AUTH-001 | Login exclusivo Google via Supabase Auth | M1 | TESTED | hosted OAuth | Cliente Google Web configurado nos Supabase Production/Preview; login real em Production retornou ao onboarding jurídico |
...
| NOTIFY-002 | EmailProvider isolado, fake dev e Resend produção | M5 | BLOCKED | provider tests/hosted | Conta e chave Resend sensível configuradas; envio real aguarda compra, DNS e verificação do domínio e definição de EMAIL_FROM |
...
| OPS-004 | Ambientes development/preview/production separados | M7 | BLOCKED | deployment | Cloudflare Production publicado em `lastro.feraimentor.workers.dev`; Supabase Production e Preview são isolados em São Paulo. Callback OAuth do novo host, secret administrativo/cron, Preview reativado, domínio/EMAIL_FROM, envio real, hosted E2E integral e restore hospedado ainda pendentes |
...
- O callback Google chega ao Supabase com `redirect_to` do novo host, mas o login completo precisa ser confirmado no navegador após incluir `https://lastro.feraimentor.workers.dev/auth/callback` na allowlist do Supabase. `SUPABASE_SECRET_KEY` ainda precisa ser gravada como secret do Worker para bootstrap e jobs; a chave não está disponível neste repositório e não será substituída por bypass de RLS.
- Supabase Preview `iyrmkcuuxhxovsbkcatq` está `INACTIVE` por inatividade no plano gratuito e deve ser reativado somente durante o restore/E2E final para economizar recursos. Um Preview da aplicação deve ser publicado apenas quando esse projeto for reativado, mantendo chaves e callbacks separados de Production.
